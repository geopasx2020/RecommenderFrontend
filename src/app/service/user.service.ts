...
export class UserService {
  ...
  getRecommendationList(userId: any): Observable<Poi[]> {
    return this.httpClient.get<Poi[]>(
      `http://localhost:8080/pois/recommendations/${userId}`
    );
  }
  ...
  createPoi(poi: Poi): Observable<Object> {
    return this.httpClient.post(`http://localhost:8080/pois`, poi);
  }
  ...
  getUserById(userId: number): Observable<User> {
    return this.httpClient.get<User>(
      `http://localhost:8080/api/v1/user/users/${userId}`
    );
  }
  ...
  public login(email: any, password: any, role: any) {
    ...
    return this.httpClient.get(
      `http://127.0.0.1:8080/api/v1/user/login/${email}/${password}`
    );
  }
  ...
  getProfileDetailsByUser(userId: any): Observable<User> {
    console.log(userId);
    return this.httpClient.get<User>(
      `http://localhost:8080/api/v1/user/users/interestings/${userId}`
    );
  }
  getPoisCategories(): Observable<Category[]> {
    return this.httpClient.get<Category[]>(
      `http://localhost:8080/interestings`
    );
  }

  createInterestingByUser(
    userId: any,
    interId: any,
    user: User
  ): Observable<Object> {
    return this.httpClient.put(
      `http://localhost:8080/api/v1/user/${userId}/interestings/${interId}`,
      user
    );
  }
  ...
  getPoiByCategory(categoryId: any): Observable<Poi> {
    return this.httpClient.get<Poi>(
      `http://localhost:8080/category/categories/${categoryId}/pois`
    );
  }
  ...
  addReview(poiId: any, review: Review): Observable<Object> {
    return this.httpClient.post(
      `http://localhost:8080/pois/${poiId}/review`,
      review
    );
  }
  ...
  getPoiByID(poiId: number): Observable<Poi> {
    return this.httpClient.get<Poi>(
      `http://localhost:8080/pois/getpoi/${poiId}`
    );
  }
  ...
  uploadImage(file: File): Observable<Object> {
    ...
    return this.httpClient.post(`http://localhost:8080/pois/images`, formData);
  }
}
